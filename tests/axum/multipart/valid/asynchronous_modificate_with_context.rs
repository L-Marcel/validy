...
use validy::axum::valid::ValidMultipart;
...
#[validate(asynchronous, context = Arc<dyn MockedService>, modificate, multipart)]
pub struct TestDTO {
	#[special(ignore)]
	#[serde(skip)]
...
pub async fn test_handle(
	...
	ValidMultipart(data): ValidMultipart<TestDTO>,
) -> Result<impl IntoResponse, (StatusCode, String)> {
	Ok((StatusCode::CREATED, Json(data)))
}

pub async fn test_two_handle(
	ValidMultipart(data): ValidMultipart<RoleDTO>,
) -> Result<impl IntoResponse, (StatusCode, String)> {
	Ok((StatusCode::CREATED, Json(data)))
}
