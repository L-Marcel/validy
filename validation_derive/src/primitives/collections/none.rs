...
pub fn create_none(input: ParseStream, field: &FieldAttributes) -> TokenStream {
	let field_name = field.get_name();
	let reference = field.get_reference();
	let content = remove_parens(input);
	let import = import_validation_functions("iter::validate_none");
	...
	quote! {
	  use #import;
		if let Err(e) = validate_none(&#reference, #items, #field_name, #code, #message) {
		  errors.push(e);
	  }
	}
}
